...
MXThread::MXThread() : mThread(NULL), mPriority(Normal), mStackSize(0), mThreadName(NULL)
{
    MATRIX_ENGINE_ASSERT(!IsRunning());
...
void Matrix::Core::MXThread::SetPriority(const Priority &p)
{
...
void Matrix::Core::MXThread::Start()
{
...
void Matrix::Core::MXThread::Suspend()
{
...
bool Matrix::Core::MXThread::IsRunning() const
{
    if (NULL != mThread)
    {
        DWORD exitCode = 0;
...
void Matrix::Core::MXThread::Sleep(DWORD dwMillseconds)
{
    ::Sleep(dwMillseconds);
}

bool Matrix::Core::MXThread::IsStopTrigger()
{
    return mStopEvent.IsTrigger();
}

void Matrix::Core::MXThread::Stop()
{
    if (mThreadState == TS_START)
    {
        MATRIX_ENGINE_ASSERT(this->IsRunning());
        MATRIX_ENGINE_ASSERT(NULL != mThread);
...
void Matrix::Core::MXThread::SetThreadName(const char *name)
{
...
const TCHAR *Matrix::Core::MXThread::GetThreadName()
{
    return nullptr;
}

DWORD THREAD_CALLBACK Matrix::Core::MXThread::ThreadProc(void *t)
{
    return 0;
}
