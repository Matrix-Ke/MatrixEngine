...
namespace Matrix
{
    namespace Core
    {
        class MATRIX_CORE_API MXSynchronize
        {
        public:
            MXSynchronize(){};
            virtual ~MXSynchronize(){};
            ...
            static unsigned int WaitAll(MXSynchronize **pSynchornize, unsigned int uiNum, bool bWaitAll, DWORD dwMilliseconds = (DWORD)-1);

            static void MXSafeOutputDebugString(const TCHAR *pString, ...);
        };

        class MATRIX_CORE_API MXCriticalSection
        {
        private:
            // windows api
        ...
        public:
            ...
            {
            private:
                MXCriticalSection &mCriticalSection;

            public:
                Locker(MXCriticalSection &inCriticalSection) : mCriticalSection(inCriticalSection)
                {
        ...
        class MATRIX_CORE_API MXSemaphore : public MXSynchronize
        {
        public:
        ...
        class MATRIX_CORE_API MXMutex : public MXSynchronize
        {
        ...
        class MATRIX_CORE_API MXEvent : public MXSynchronize
        {
