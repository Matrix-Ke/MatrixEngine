...
VSREAL Matrix::Primitive::Line3::SquaredDistance(const Matrix::Math::VSVector3& Point, VSREAL& fLineParameter) const
{
	Matrix::Math::VSVector3 Diff = Point - m_Orig;
...
{
	VSREAL SquareDis;
	Matrix::Math::VSVector3 Diff = m_Orig - Line.m_Orig;
...
/*----------------------------------------------------------------*/
VSREAL Matrix::Primitive::Line3::SquaredDistance(const VSTriangle3& Triangle, VSREAL& fLineParameter, VSREAL fTriangleParameter[3]) const
{
	Matrix::Math::VSVector3 TrianglePoint[3];
	Triangle.GetPoint(TrianglePoint);

	/*Matrix::Math::VSVector3 Diff = TrianglePoint[0] - m_Orig;
	Matrix::Math::VSVector3 Edge0 = TrianglePoint[1] - TrianglePoint[0];
	Matrix::Math::VSVector3 Edge1 = TrianglePoint[2] - TrianglePoint[0];
...
VSREAL Matrix::Primitive::Line3::SquaredDistance(const Rectangle3& Rectangle, VSREAL& fLineParameter, VSREAL fRectangleParameter[2]) const
{
	...
	VSREAL fSqrDist = VSMAX_REAL;
	VSREAL fSqrDistTmp;
	Matrix::Math::VSVector3 r[4];

	Matrix::Math::VSVector3 A[2];
	Rectangle.GetA(A);
	VSREAL fA[2];
	Rectangle.GetfA(fA);

	Matrix::Math::VSVector3 Temp0 = Rectangle.GetCenter() + A[0] * fA[0];
	Matrix::Math::VSVector3 Temp1 = Rectangle.GetCenter() + A[0] * (-fA[0]);
	r[0] = Temp0 + A[1] * fA[1];
	r[1] = Temp0 + A[1] * (-fA[1]);
	r[2] = Temp1 + A[1] * (-fA[1]);
	r[3] = Temp1 + A[1] * fA[1];
...
/*----------------------------------------------------------------*/
VSREAL Matrix::Primitive::Line3::SquaredDistance(const VSOBB3& OBB, VSREAL& fLineParameter, VSREAL fOBBParameter[3]) const
{

	return OBB.SquaredDistance(*this, fOBBParameter, fLineParameter);
}
/*----------------------------------------------------------------*/
VSREAL Matrix::Primitive::Line3::Distance(const Sphere3& Sphere, VSREAL& fLineParameter, Matrix::Math::VSVector3& SpherePoint) const
{
	return Sphere.Distance(*this, SpherePoint, fLineParameter);
}
/*----------------------------------------------------------------*/
VSREAL Matrix::Primitive::Line3::Distance(const Plane3& Plane, Matrix::Math::VSVector3& LinePoint, Matrix::Math::VSVector3& PlanePoint) const
{
