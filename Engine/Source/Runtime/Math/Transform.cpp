...
void VSTransform::ApplyForward(const Vector3& In, Vector3& Out, unsigned int uiTransformType)
{
...
void VSTransform::ApplyInverse(const Vector3& In, Vector3& Out, unsigned int uiTransformType)
{

	if (uiTransformType == TT_POS)
	{
		const Matrix4& mat = GetCombineInverse();
		Out = In * mat;
	}
	else if (uiTransformType == TT_DIR)
	{
...
void VSTransform::SetMatrix(const Matrix4& VSMat)
{
