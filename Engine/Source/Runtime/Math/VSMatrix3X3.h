...
namespace Matrix
{
	...
	{
		//矩阵
		class  MATRIX_MATH_API VSMatrix3X3
		{
		public:
			union
			{
			...
			inline void TransposeOf(const VSMatrix3X3& Matirx);					//转置
			inline void InverseOf(const VSMatrix3X3& Matirx);					//求逆
			inline VSREAL Det()const;											//求判别式
			inline VSMatrix3X3 operator * (const VSMatrix3X3& Matirx)const;		// 矩阵相乘
			...
			void GetScaleAndRotator(VSVector3& Scale);
			...
			const static VSMatrix3X3 ms_3DMax_To_Engine;
