...
{
	ms_NULLShaderKey = MX_NEW VSShaderKey();
	return 1;
}
...
void VSShaderKey::SetMaterialHShaderKey(VSShaderKey* pKey, MaterialShaderPara& MSPara)
{
	...
	if (uiPassType == VSPass::PT_MATERIAL || uiPassType == VSPass::PT_NORMALDEPTH || uiPassType == VSPass::PT_POINT_CUBE_SHADOW || uiPassType == VSPass::PT_POINT_VOLUME_SHADOW || uiPassType == VSPass::PT_DIRECT_VOLUME_SHADOW || uiPassType == VSPass::PT_SHADOW || uiPassType == VSPass::PT_DUAL_PARABOLOID_SHADOW || uiPassType == VSPass::PT_INDIRECT)
	{
...
void VSShaderKey::SetMaterialPShaderKey(VSShaderKey* pKey, MaterialShaderPara& MSPara)
{
	...
	else if (uiPassType == VSPass::PT_SHADOW || uiPassType == VSPass::PT_POINT_CUBE_SHADOW || uiPassType == VSPass::PT_POINT_CUBE_SHADOW || uiPassType == VSPass::PT_DUAL_PARABOLOID_SHADOW)
	{
		unsigned int uiVertexFormatCode = 0;
		if (MSPara.pGeometry)
		{
...
void VSShaderKey::GetDefine(Container::MMap<Container::MString, Container::MString>& Define)
{
	Define.Clear();
