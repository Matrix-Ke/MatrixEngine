...
bool VSOrenNayarShaderFunction::GetFunctionString(Container::MString &OutString, MaterialShaderPara &MSPara) const
{
    ...
    else
    {
        ...
        for (unsigned int i = 0; i < VSLight::LT_MAX; i++)
        {
            ...
            if (i == VSLight::LT_DIRECTION)
            {
                ...
                VSRenderer::ms_pRenderer->OrenNayarDirectionalLight(iLightNum[i], m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(), m_pInput[IN_ROUGHNESS_SQUARED]->GetNodeName().GetString(), *VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"), UseLookUpTable(), ShadowStringArray[i], OutString);
            }
            else if (i == VSLight::LT_POINT)
            {

                VSRenderer::ms_pRenderer->OrenNayarPointLight(iLightNum[i], m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(), m_pInput[IN_ROUGHNESS_SQUARED]->GetNodeName().GetString(), *VSShaderStringFactory::ms_WorldPos, *VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"), UseLookUpTable(), ShadowStringArray[i], OutString);
            }
            else if (i == VSLight::LT_SPOT)
            {

                VSRenderer::ms_pRenderer->OrenNayarSpotLight(iLightNum[i], m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(), m_pInput[IN_ROUGHNESS_SQUARED]->GetNodeName().GetString(), *VSShaderStringFactory::ms_WorldPos, *VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"), UseLookUpTable(), ShadowStringArray[i], OutString);
            }
        }
        OutString += _T(";\n");
    }
