    ...
    else
    {
        ...
        for (unsigned int i = 0; i < VSLight::LT_MAX; i++)
        {
            ...
            if (i == VSLight::LT_DIRECTION)
            {
                ...
                VSRenderer::ms_pRenderer->DirectionalLight(iLightNum[i], m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(), m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(), m_pInput[IN_SPECULAR_POW]->GetNodeName().GetString(), *VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"), ShadowStringArray[i], OutString);
            }
            else if (i == VSLight::LT_POINT)
            {

                VSRenderer::ms_pRenderer->PointLight(iLightNum[i], m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(), m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(), m_pInput[IN_SPECULAR_POW]->GetNodeName().GetString(), *VSShaderStringFactory::ms_WorldPos, *VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"), ShadowStringArray[i], OutString);
            }
            else if (i == VSLight::LT_SPOT)
            {

                VSRenderer::ms_pRenderer->SpotLight(iLightNum[i], m_pInput[IN_DIFFUSE_COLOR]->GetNodeName().GetString(), m_pInput[IN_SPECULAR_COLOR]->GetNodeName().GetString(), m_pInput[IN_SPECULAR_POW]->GetNodeName().GetString(), *VSShaderStringFactory::ms_WorldPos, *VSShaderStringFactory::ms_WorldNormal, _T("WorldCameraDir"), ShadowStringArray[i], OutString);
            }
        }
        OutString += _T(";\n");
    }
