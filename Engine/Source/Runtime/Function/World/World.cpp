...
VSWorld::VSWorld()
{
	ENGINE_ASSERT(ms_pWorld == NULL);
	ms_pWorld = this;
	VSSceneMap* pSceneMap = MX_NEW VSSceneMap("Main");
...
VSSceneMap* VSWorld::CreateScene(const TCHAR* pName)
{
	...
	VSSceneMap* pSceneMap = MX_NEW VSSceneMap(pName);
	VSResourceManager::AddGCObject(pSceneMap);
	m_SceneArray.AddElement(pSceneMap);
	return pSceneMap;
}
...
VSActor* VSWorld::CreateActor(const TCHAR* ActorPath, const Math::Vector3& Pos, const Math::Matrix3& Rotate, const Math::Vector3& Scale, VSSceneMap* pSceneMap)
{
	...
	if (pResource)
	{
		unsigned int ResourceType = pResource->GetResourceType();
...
void VSWorld::AttachCaptureViewFamilyToCamera(VSCameraActor* pCameraActor,
	unsigned int uiCaptureViewFamilyType, const Container::MString& ViewFamilyName,
	unsigned int uiWidth, unsigned int uiHeight,
	Container::MArray<Container::MString>& SceneMapName,
	...
	VSPostEffectSetR* pPostEffectSet)
{
	...
	if (pCamera)
	{
		VSViewFamliyInfo* pInfo = MX_NEW VSViewFamliyInfo();

		VSViewFamily* pViewFamily = NULL;
		if (uiCaptureViewFamilyType == VSCaptureViewFamily::VT_CAPTURE_2D_CAPTURE_NORMAL)
		{
			pViewFamily = MX_NEW VS2DCaptureViewFamily(ViewFamilyName, uiWidth, uiHeight, pCamera, pPostEffectSet, RenderMethodRTTIName);
		}
		else if (uiCaptureViewFamilyType == VSCaptureViewFamily::VT_CAPTURE_CUB_CAPTURE_NORMAL)
		{
			pViewFamily = MX_NEW VSCubCaptureViewFamily(ViewFamilyName, uiWidth, uiHeight, pCamera, pPostEffectSet, RenderMethodRTTIName);
		}
...
void VSWorld::AttachWindowViewFamilyToCamera(VSCameraActor* pCameraActor,
	unsigned int uiWindowViewFamilyType, const Container::MString& ViewFamilyName,
	Container::MArray<Container::MString>& SceneMapName,
	const TCHAR* RenderMethodRTTIName,
	int iWindowID,
	VSPostEffectSetR* pPostEffectSet)
{
	...
	if (pCamera)
	{
		...
		{
			pViewFamily = MX_NEW VSWindowViewFamily(ViewFamilyName, pCamera, pPostEffectSet, RenderMethodRTTIName, iWindowID);
		}
		pInfo->m_SceneMapName = SceneMapName;
