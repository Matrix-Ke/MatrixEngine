...
bool VSRenderer::SetRenderTarget(VSRenderTarget *pRenderTarget, unsigned int i)
{
    ...
    if (m_uiCurRTWidth != pRenderTarget->GetWidth() || m_uiCurRTHeight != pRenderTarget->GetHeight() || m_uiCurRTMultisample != pRenderTarget->GetMulSample())
    {
...
bool VSRenderer::ChangeScreenSize(unsigned int uiWidth, unsigned int uiHeight, bool bWindow)
{
    ...
    if (!bWindow)
    {
        ...
        {
            if (pAdapterInfo[0].DisplayMode[i].uiHeight == uiHeight && pAdapterInfo[0].DisplayMode[i].uiWidth == uiWidth && pAdapterInfo[0].DisplayMode[i].uiBitFormat == VSRenderer::SFT_A8R8G8B8)
            {
                bFlag = true;
                break;
            }
