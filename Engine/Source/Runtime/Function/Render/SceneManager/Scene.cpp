...
void VSScene::Update(double dAppTime)
{
	if (m_bIsBuild == true)
	{
		...
		if (m_pStaticRoot)
		{
			ENQUEUE_LAMBDA_UPDATE_COMMAND(VSUpdateStaticNode)
			([this, dAppTime]()
			{
				m_pStaticRoot->UpdateAll(dAppTime);
			});
		}

		for (unsigned int i = 0; i < m_pDynamic.GetNum(); i++)
		{
			ENQUEUE_LAMBDA_UPDATE_COMMAND(VSUpdateDynamicNode)
			([this, dAppTime,i]()
			{
				m_pDynamic[i]->UpdateAll(dAppTime);
			});
		}
	}
	else
...
void VSScene::GetStreamResource(const VSVector3& CameraPos, const VSVector3W& ProjectInfo)
{
	...
	([this, CameraPos, ProjectInfo,&FreeSlot]()
	{
		for (unsigned int i = 0; i < m_ObjectNodes.GetNum(); i++)
		{
			StreamInformation_TYPE StreamInformation;
			StreamInformation.CameraPos = CameraPos;
			StreamInformation.ProjectInfo = ProjectInfo;
			m_ObjectNodes[i]->GetStreamResource(FreeSlot.pResourceProxy, StreamInformation);
		}
	});
...
bool VSQuadNode::RecursiveBuild(const VSArray<VSSpatial *> &pObjectArray)
{
	...
	for (unsigned int i = 0 ; i < pObjectArray.GetNum() ; i++)
	{
		if (pObjectArray[i])
		{
			...
			{
				Total = Total.MergeAABB(WorldAABB);
			}
