...
#define CREATE_UI_PROPERTY(type, Name) VSEditorUIPropertyCreator::GetInstance().CreateUIProperty(type, Name);
	...
	public:
		...
		virtual void GetValueString(Container::MString& IntString)
		{
			...
			{
				IntString = Container::IntToString(*m_pData);
			}
			...
			{
				IntString = Container::RealToString(*m_pData);
			}
#endif
		}
	...
	public:
			...
			: VSEditorProperty(Name, pOwner)
		{
			...
			if (bRange)
			{
				if (Min.x > Max.x)
				{
					Math::Swap(Max.x, Min.x);
				}
				if (Min.y > Max.y)
				{
					Math::Swap(Max.y, Min.y);
				}
				if (Min.z > Max.z)
				{
			...
			m_pNameX = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("X"));
			m_pNameY = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Y"));
			m_pNameZ = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Z"));

			m_pTextX = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("X"));
			m_pTextX->SetOwner(this);

			m_pTextY = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("Y"));
			m_pTextY->SetOwner(this);

			m_pTextZ = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("Z"));
			m_pTextZ->SetOwner(this);

			if (bRange)
			{
				m_pSliderX = (VSESlider*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_SLIDER, Name + _T("X"));
				m_pSliderX->SetOwner(this);
				m_pSliderX->SetRange(0, int((Max.x - Min.x) / Step.x), 1);

				m_pSliderZ = (VSESlider*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_SLIDER, Name + _T("Z"));
				m_pSliderZ->SetOwner(this);
				m_pSliderZ->SetRange(0, int((Max.z - Min.z) / Step.z), 1);

				m_pSliderY = (VSESlider*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_SLIDER, Name + _T("Y"));
		...
		virtual void SetValue()
		{
			...
			Container::MString RealStringY = Container::RealToString(fDataY);
			m_pTextY->SetValue((void*)&RealStringY);

			VSREAL fDataX = m_pVector3->x;
			Container::MString RealStringX = Container::RealToString(fDataX);
			m_pTextX->SetValue((void*)&RealStringX);

			VSREAL fDataZ = m_pVector3->z;
			Container::MString RealStringZ = Container::RealToString(fDataZ);
	...
	{
	public:
		VSETransformProperty(Math::VSTransform* pTransform, Container::MString& Name, MObject* pOwner)
			: VSEditorProperty(Name, pOwner)
		{
			m_pTransform = pTransform;

			m_pTranslateName = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Translate"));
			m_pRotationName = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Rotation"));
			m_pScaleName = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Scale"));

			m_pTranslateNameX = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("X"));
			m_pTranslateNameY = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Y"));
			m_pTranslateNameZ = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Z"));

			m_pRotationNameX = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("X"));
			m_pRotationNameY = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Y"));
			m_pRotationNameZ = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Z"));

			m_pScaleNameX = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("X"));
			m_pScaleNameY = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Y"));
			m_pScaleNameZ = (VSELable*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_LABLE, _T("Z"));

			m_pTranslateTextX = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("TranslateX"));
			m_pTranslateTextX->SetOwner(this);

			m_pTranslateTextY = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("TranslateY"));
			m_pTranslateTextY->SetOwner(this);

			m_pTranslateTextZ = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("TranslateZ"));
			m_pTranslateTextZ->SetOwner(this);

			m_pRotationTextX = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("RotationX"));
			m_pRotationTextX->SetOwner(this);

			m_pRotationTextZ = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("RotationY"));
			m_pRotationTextZ->SetOwner(this);

			m_pRotationTextY = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("RotationZ"));
			m_pRotationTextY->SetOwner(this);

			m_pScaleTextX = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("ScaleX"));
			m_pScaleTextX->SetOwner(this);

			m_pScaleTextY = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("ScaleX"));
			m_pScaleTextY->SetOwner(this);

			m_pScaleTextZ = (VSEText*)CREATE_UI_PROPERTY(VSEditorSingle::CUT_TEXT, Name + _T("ScaleX"));
		...
		virtual void SetValue()
		{
			...
			Container::MString RealStringTranX = Container::RealToString(fTranDataX);
			m_pTranslateTextX->SetValue((void*)&RealStringTranX);

			VSREAL fTranDataY = Tran.y;
			Container::MString RealStringTranY = Container::RealToString(fTranDataY);
			m_pTranslateTextY->SetValue((void*)&RealStringTranY);

			VSREAL fTranDataZ = Tran.z;
			Container::MString RealStringTranZ = Container::RealToString(fTranDataZ);
			m_pTranslateTextZ->SetValue((void*)&RealStringTranZ);

			Math::Matrix3 Mat = m_pTransform->GetRotate();
			VSREAL X, Y, Z;
			Mat.GetEuler(Z, X, Y);
			;

			Container::MString RealStringRotateX = Container::RealToString(X);
			m_pRotationTextX->SetValue((void*)&RealStringRotateX);

			Container::MString RealStringRotateY = Container::RealToString(Y);
			m_pRotationTextY->SetValue((void*)&RealStringRotateY);

			Container::MString RealStringRotateZ = Container::RealToString(Z);
			m_pRotationTextZ->SetValue((void*)&RealStringRotateZ);

			Math::Vector3 Scale = m_pTransform->GetScale();

			VSREAL fScaleDataX = Scale.x;
			Container::MString RealStringScaleX = Container::RealToString(fScaleDataX);
			m_pScaleTextX->SetValue((void*)&RealStringScaleX);

			VSREAL fScaleDataY = Scale.y;
			Container::MString RealStringScaleY = Container::RealToString(fScaleDataY);
			m_pScaleTextY->SetValue((void*)&RealStringScaleY);

			VSREAL fScaleDataZ = Scale.z;
			Container::MString RealStringScaleZ = Container::RealToString(fScaleDataZ);
	...
	template <>
	inline VSEditorElement* CreateEElement<Math::VSTransform>(Math::VSTransform& Value, Container::MString& Name, MObject* pOwner, bool bRange, Math::VSTransform& Max, Math::VSTransform& Min, Math::VSTransform& fStep)
	{
	...
	*/
	...
	void CreateEditorElement(Container::MArray<T, VSMemManagerClass>& Value, MObject* pOwner, VSECollection* pParent, Container::MString& Name, bool Range = false, T Max = T(), T Min = T(), T fStep = T())
	{
		...
		{
			Container::MString NewName = Name + Container::IntToString(i);
