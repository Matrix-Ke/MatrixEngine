...
VSName::VSName(const Container::MString& String, unsigned int uiID)
{
	m_uiID = uiID;
	m_String.Clear();
	m_String = String;
...
void VSUsedName::Archive(MStream& Stream)
{
	if (Stream.GetStreamFlag() == MStream::AT_LOAD)
	{
		...
		if (String == Container::MString::ms_StringNULL)
		{
			m_pName = NULL;
		}
		else
...
namespace Matrix
{
	...
	bool operator==(const VSUsedName& Name, const Container::MString& String)
	{
		if (!Name.m_pName)
		{
			return 0;
