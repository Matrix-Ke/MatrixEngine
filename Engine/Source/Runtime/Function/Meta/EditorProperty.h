...
namespace Matrix
{
	...
	// 	{
	// 	protected:
	// 		QTCheckBox(Container::MString & Name) :VSECheckBox(Name)
	// 		{
	// 			m_pCheckBox = MX_NEW QCheckBox(Name);
...
#define CREATE_UI_PROPERTY(type, Name) VSEditorUIPropertyCreator::GetInstance().CreateUIProperty(type, Name);
	...
	{
	public:
		VSEVector3Property(VSVector3* pVector3, Container::MString& Name, VSObject* pOwner, bool bRange, VSVector3 Max, VSVector3 Min, VSVector3 Step)
			: VSEditorProperty(Name, pOwner)
		{
	...
	template <>
	inline VSEditorElement* CreateEElement<VSVector3>(VSVector3& Value, Container::MString& Name, VSObject* pOwner, bool bRange, VSVector3& Max, VSVector3& Min, VSVector3& fStep)
	{
