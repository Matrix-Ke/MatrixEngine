...
VSFileMonitor::VSFileMonitor(VSMonitor *pOwner)
    : m_isUseRelativePath(false), m_pReadBuffer(NULL)
{
    m_pOwner = pOwner;
}
...
//------------------------------------------------------------------------------
void VSFileMonitor::NotifyHandler()
{
    FILE_NOTIFY_INFORMATION *pInfo = (FILE_NOTIFY_INFORMATION *)m_pReadBuffer;
...
//------------------------------------------------------------------------------
const TCHAR *VSFileMonitor::GetThreadName()
{
...
//------------------------------------------------------------------------------
void VSResourceMonitor::Update(double tick)
{
...
//------------------------------------------------------------------------------
void VSResourceMonitor::Close()
{
    m_fileMonitor->Stop();
    VSMAC_DELETE(m_fileMonitor);
}
//------------------------------------------------------------------------------
void VSResourceMonitor::OnFileChanged(const FileChangedArgs &args)
{
