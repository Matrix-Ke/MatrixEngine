...
Matrix::Core::UEWin32MemoryAlloc::UEWin32MemoryAlloc()
{
	...
	// pagesize首位为1其他位需要为0；
	ENGINE_ASSERT(!(PageSize & (PageSize - 1)));
	...
	for (DWORD i = 0; i < POOL_MAX; i++)
	{
		...
		for (Index = 0; PoolTable[Index].BlockSize < i; Index++)
			;
		ENGINE_ASSERT(Index < POOL_CATEGORY);
	...
	//判断是否数据对应的上
	ENGINE_ASSERT(POOL_MAX - 1 == PoolTable[POOL_CATEGORY - 1].BlockSize);
}
...
void* Matrix::Core::UEWin32MemoryAlloc::Allocate(USIZE_TYPE uSize, USIZE_TYPE uiAlignment, bool bIsArray)
{
	...
	if (uSize < POOL_MAX)
	{
		...
		ENGINE_ASSERT(uSize < Table->BlockSize);
		...
		if (!Pool)
		{
			...
			ENGINE_ASSERT(Blocks >= 1);
			ENGINE_ASSERT(Blocks * Table->BlockSize <= Bytes);
		...
		Pool->Taken++;
		ENGINE_ASSERT(Pool->pAvailableBlock);
		ENGINE_ASSERT(Pool->pAvailableBlock->Blocks > 0);
...
void DebugMemoryAlloc::Deallocate(char* pcAddr, USIZE_TYPE uiAlignment, bool bIsArray)
{
	...
	pcAddr -= sizeof(unsigned int);
	unsigned int* pBeginMask = (unsigned int*)(pcAddr);
	ENGINE_ASSERT(*pBeginMask == BEGIN_MASK);
	pcAddr -= sizeof(Block);

	Block* pBlock = (Block*)pcAddr;
	RemoveBlock(pBlock); //将deallocate的内存块移到unused部分去

	ENGINE_ASSERT(pBlock->mbArray == bIsArray);
	ENGINE_ASSERT(mNumBlocks > 0 && mNumBytes >= pBlock->mSize);
	bool bAlignment = (uiAlignment > 0) ? true : false;
	ENGINE_ASSERT(pBlock->mbAlignment == bAlignment);
	//判断尾标识
	unsigned int* pEndMask = (unsigned int*)(pcAddr + sizeof(Block) + sizeof(unsigned int) + pBlock->mSize);
	ENGINE_ASSERT(*pEndMask == END_MASK);
...

{
	...
	ENGINE_ASSERT(uiDefaultChunkSize > sizeof(FTaggedMemory));
	Top = NULL;
	End = NULL;
...
Matrix::Core::StackMemoryManager::~StackMemoryManager()
{
	...
	//核验是否取消分配完毕
	ENGINE_ASSERT(NumMarks == 0);
}
void* Matrix::Core::StackMemoryManager::Allocate(USIZE_TYPE uSize, USIZE_TYPE uiAlignment, bool bIsArray)
{
	ENGINE_ASSERT(uSize >= 0);
	if (uiAlignment > 0)
	{
		ENGINE_ASSERT((uiAlignment & (uiAlignment - 1)) == 0);
	}

	ENGINE_ASSERT(Top <= End);
	ENGINE_ASSERT(NumMarks > 0);
	//从当前chunk里分配空间
	BYTE* Result = Top;
	if (uiAlignment > 0)
